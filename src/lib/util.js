...
var World = require('./world')
  ...
  // color herlpers
  , fmtErr = compose(pre, inverse, bold, red)

function buildFunction(x, id, key) {
  var context = require('./context')
    , code = '(' + x.__function__ + ')'
    , script = new vm.Script(code, {filename: id + '.' + key})
    , f = function() {
            vmLogger.debug(code)
            return script.runInContext(context).apply(this, arguments)
          }

  modifyObject(f, function(property, accessor) {

    function source() {
      return x.__function__
    }
